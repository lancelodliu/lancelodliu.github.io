...
### launch
未配置`nodaemon`时调用此函数。实际是使用`reactor`调用`twistd.run()`，同时还写了个pid到文件。
...
`runApp(config)`相当于调用`_SomeApplicationRunner(config).run()`：
```python
# line 654
...
而`WindowsApplicationRunner`继承自`app.ApplicationRunner`，此定义仍在这个文件中：
